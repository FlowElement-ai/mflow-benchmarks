...
if TYPE_CHECKING:
    from m_flow.memory.episodic.models import FragmentDigest
...
async def _task_extract_entity_names(
    ...
    if cache_was_populated:
        logger.info(
            f"[episodic] Concept extraction from routing cache: {len(cached_entity_names)} names "
        ...
        logger.info(
            f"[episodic] Concept extraction complete: "
...
async def execute_phase0a(ctx: EpisodeContext) -> Phase0AResult:
    ...
    try:
        ...
        if isinstance(parallel_results[0], Exception):
            logger.error(f"[episodic] Phase 0A: Concept extraction failed: {parallel_results[0]}")
        else:
